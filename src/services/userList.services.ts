...
export class UserListService {
    ...
    public async createAdminUser(userInfo?: IUserInfo): Promise<SpinalNode> {
        ...
        return this._addUserToContext({ name: userName, userName, type: USER_TYPES.ADMIN, userType: USER_TYPES.ADMIN }, new Model({ userName, password: await this._hashPassword(password) }), true);
    }
    ...
    public async authAdmin(user: IUserCredential): Promise<{ code: number; data: any | string }> {
        ...
        if (!node) return { code: HTTP_CODES.INTERNAL_ERROR, data: "bad username and/or password" };

        const element = await node.getElement(true);
        const success = await this._comparePassword(user.password, element.password.get());
        if (!success) return { code: HTTP_CODES.UNAUTHORIZED, data: "bad username and/or password" };
    ...
    private async _addUserToContext(info: { [key: string]: any }, element?: spinal.Model, isAdmin: boolean = false): Promise<SpinalNode> {
        ...
        return this.context.addChildInContext(node, relationName, PTR_LST_TYPE, this.context);
    }
